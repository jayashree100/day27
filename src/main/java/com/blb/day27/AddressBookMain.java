...
public class AddressBookMain {
	...
	private static void view_by_state_or_city_sorted() {
		...
		switch (type) {
		case "state": {
			List<Contacts> sorted_by_state = contactList.stream().sorted(Comparator.comparing(Contacts::getState))
		...
		case "city": {
			List<Contacts> sorted_by_state = contactList.stream().sorted(Comparator.comparing(Contacts::getCity))
					.collect(Collectors.toList());
			sorted_by_state.forEach(contact -> {
				System.out.println("\nFirstName - " + contact.getFirstname() + "\nLastname -  " + contact.getLastname()
						+ "\nAddress -  " + contact.getAddress() + "\nCity -  " + contact.getCity() + "\nState -  "
		...
		case "zip": {
			List<Contacts> sorted_by_state = contactList.stream().sorted(Comparator.comparing(Contacts::getZip))
					.collect(Collectors.toList());
			sorted_by_state.forEach(contact -> {
				System.out.println("\nFirstName - " + contact.getFirstname() + "\nLastname -  " + contact.getLastname()
						+ "\nAddress -  " + contact.getAddress() + "\nCity -  " + contact.getCity() + "\nState -  "
	...
	private static void search_by_city_or_state_get_multiple_person(String input) {
		if (contactList.stream().anyMatch(
				search -> search.getState().equalsIgnoreCase(input) || search.getCity().equalsIgnoreCase(input))) {
	...
	private static void add_contact_to_list(int index) {
		...
		AddressBook.book.put(firstname,
				new Contacts(firstname, lastname, address, city, state, zip, phone_number, email));
	}
